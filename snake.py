...
class Snake:
    ...
    def __init__(self,_geometry,_head,_direction):
        ...
        self.newhead = [self.head[0] + self.direction[0], self.head[1] + self.direction[1]]
...
#guidage sur x
    ...
    if ORDI.newhead in rocks and (ORDI.direction == [1,0] or ORDI.direction == [-1,0]):
        if ORDI.head[0] <= 14:
            ORDI.direction = [0,1]
...
#guidage sur y (peut être là le pb, pcq on peut pu toucher aux x une fois qu'on y est)
    ...
    ORDI.newhead = [ORDI.head[0]+ORDI.direction[0],
                    ORDI.head[1]+ORDI.direction[1]]

    print(f"{ORDI.newhead} newhead après")
    print(f"{ORDI.head} head après")
    
    if (ORDI.newhead in rocks) or ORDI.newhead[0] < 0 or ORDI.newhead[0] > 29 or ORDI.newhead[1] < 0 or ORDI.newhead[1] > 29:
        chng_dir()
